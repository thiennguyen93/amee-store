...
async function validatePackage(dirArg, { schemas, index }) {
  ...
  // 3. Everything validate_manifest in skin.rs would reject.
  ...
  if (!manifest.version) {
    ...
    if (published && compareSemver(manifest.version, published.version) <= 0) {
      fail(`version ${manifest.version} is not greater than the published ${published.version} — bump it`);
    }
  }
