...
/// Mirrors `MAX_EXTRACTED_BYTES` in the Amee repo's src-tauri/src/skin.rs.
/// Amee refuses to import a package that exceeds this once extracted, so a
/// package that trips it here would be un-installable anyway.
export const MAX_EXTRACTED_BYTES = 25 * 1024 * 1024;

/// Ids that a built-in skin already occupies. `skin_dir` in skin.rs prefers
/// the built-in copy on a collision, so an installed skin sharing one of
/// these would be silently shadowed and unreachable forever.
...
/// bound is written on the JS side (and the one place to update when
/// skin.rs's MIN_DIM/MAX_DIM change).
...
/// Mirror of `validate_manifest` in the Amee repo's src-tauri/src/skin.rs.
/// Every branch below corresponds to one `return Err(...)` there, in the same
/// order, so the two can be diffed side by side. Anything Amee would reject at
/// import time must be rejected here, or a skin merges into the store and then
/// fails to install.
